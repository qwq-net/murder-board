...
type StickyData = { title: string; text: string; color?: StickyColor };

// タイムラインの 1 行。time は "HH:MM" を想定するが自由入力を許し、不正な時刻の行は末尾に並ぶ。
// color は付箋と同じパレットで、未設定なら種別既定のアクセント色で描画される
export type TimelineEntry = { id: string; time: string; text: string };
type TimelineData = { title: string; entries: TimelineEntry[]; color?: StickyColor };

// リストメモの 1 行。text は作成直後の空行のような空文字を許す。並び順は登録順のまま。
// color の意味はタイムラインと同じ
export type ListEntry = { id: string; text: string };
type ListData = { title: string; entries: ListEntry[]; color?: StickyColor };

// キーワードメモ。行の形も操作もリストメモと同じだが、登録された言葉は
// 他ノードの本文中で完全一致の検索リンクに置き換わる
type KeywordData = { title: string; entries: ListEntry[]; color?: StickyColor };
...
type CharacterData = { title: string; entries: CharacterEntry[]; color?: StickyColor };
...
type ActionLogData = { title: string; entries: ActionEntry[]; color?: StickyColor };

// スタックノードの中身。本文を持たず、タイトルだけを持つ入れ物。
// 子ノードは parentId で所属し、順番は子の相対 y 座標の昇順そのもので表す。
// color の意味はタイムラインと同じで、未設定なら無彩色の既定枠になる
type StackData = { title: string; color?: StickyColor };
...
// 新規作成時に色を持つ種別の既定カラー。パネル系ノードの未設定時の描画は
// index.css の識別色変数が担うため、ここには載せない
export const DEFAULT_NODE_COLORS = {
  sticky: "gray",
  character: "green",
} satisfies Record<"sticky" | "character", StickyColor>;
